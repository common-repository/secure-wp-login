...
/*
 * Plugin Name: Secure wp-login.php
 * Plugin URI:  https://www.wpbuild.me/secure-wp-login/
 * Description: You can change your website wp-admin.php to any thing you want to quick and simple way.
 * Version:     1.0
 * Author:      Neil Patel
 * Author URI:  https://www.wpbuild.me/secure-wp-login/faq/
 * Text Domain: Secure-wp-login
 * Domain Path:
 * Network:
 * License:     GPL-3.0+
...
if ( defined( 'ABSPATH' ) && ! class_exists( 'Secure_WP_Login' ) ) {

	class Secure_WP_Login {
		...
		public function __construct() {
			...
			register_uninstall_hook( $this->basename(), array( 'Secure_WP_Login', 'uninstall' ) );
		...
		public function admin_notices_incompatible() {
			echo '<div class="error"><p>' . sprintf( __( 'Please upgrade to the latest version of WordPress to activate %s.', 'Secure-wp-login' ), '<strong>' . __( 'Secure wp-login.php', 'Secure-wp-login' ) . '</strong>' ) . '</p></div>';
		}
		...
		public function wpmu_options() {
			...
			$out .= '<h3>' . __( 'Secure wp-login.php', 'Secure-wp-login' ) . '</h3>';
			$out .= '<p>' . __( 'This option allows you to set a networkwide default, which can be overridden by individual sites. Simply go to to the site’s permalink settings to change the url.', 'Secure-wp-login' ) . '</p>';
			$out .= '<table class="form-table">';
				$out .= '<tr valign="top">';
					$out .= '<th scope="row">' . __( 'Networkwide default', 'Secure-wp-login' ) . '</th>';
		...
		public function admin_init() {
			...
			add_settings_section(
				'Secure-wp-login-section',
				__( 'Secure wp-login.php', 'Secure-wp-login' ),
			...
			add_settings_field(
				'rwl-page',
				'<label for="rwl-page">' . __( 'Login url', 'Secure-wp-login' ) . '</label>',
		...
		public function rwl_section_desc() {
			...
			if ( is_multisite() && is_super_admin() && is_plugin_active_for_network( $this->basename() ) ) {
				$out .= '<p>' . sprintf( __( 'To set a networkwide default, go to %s.', 'Secure-wp-login' ), '<a href="' . network_admin_url( 'settings.php#rwl-page-input' ) . '">' . __( 'Network Settings', 'Secure-wp-login' ) . '</a>') . '</p>';
			}
		...
		public function admin_notices() {
			...
			if ( ! is_network_admin() && $pagenow === 'options-permalink.php' && isset( $_GET['settings-updated'] ) ) {
				echo '<div class="updated"><p>' . sprintf( __( 'Your login page is now here: %s. Bookmark this page!', 'Secure-wp-login' ), '<strong><a href="' . $this->new_login_url() . '">' . $this->new_login_url() . '</a></strong>' ) . '</p></div>';
			}
		}

		public function plugin_action_links( $links ) {
			if ( is_network_admin() && is_plugin_active_for_network( $this->basename() ) ) {
				array_unshift( $links, '<a href="' . network_admin_url( 'settings.php#rwl-page-input' ) . '">' . __( 'Settings', 'Secure-wp-login' ) . '</a>' );
			} elseif ( ! is_network_admin() ) {
				array_unshift( $links, '<a href="' . admin_url( 'options-permalink.php#rwl-page-input' ) . '">' . __( 'Settings', 'Secure-wp-login' ) . '</a>' );
			}
		...
		public function plugins_loaded() {
			...
			load_plugin_textdomain( 'Secure-wp-login' );
			...
			if (
				...
				wp_die( __( 'This feature is not enabled.', 'Secure-wp-login' ) );
			}
		...
		public function wp_loaded() {
			...
			if ( is_admin() && ! is_user_logged_in() && ! defined( 'DOING_AJAX' ) ) {
				wp_die( __( 'You must log in to access the admin area.', 'Secure-wp-login' ) );
			}
